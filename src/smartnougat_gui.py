...
class SmartNougatGUI:
    ...
    def run_smartnougat(self):
        try:
            ...
            cmd = [sys.executable, "src/smartnougat_0715.py", self.selected_file.get()]
    ...
    def create_standalone_html(self):
        ...
        try:
            ...
            base_name = Path(self.output_folder.get()).name.split('_')[0]
            parent_dir = Path(self.output_folder.get()).parent
            
            # Debug: Show what we're looking for
            self.output_queue.put(("info", f"Looking for output in: {parent_dir}"))
            ...
            for folder in os.listdir(parent_dir):
                if folder.startswith(base_name):
                    ...
                    if "_smartnougat_" in folder:
                        actual_output_dir = os.path.join(parent_dir, folder)
                        break
            
            # If not found with _smartnougat_, try other patterns
            if not actual_output_dir and found_folders:
                # Sort by modification time and get the most recent
                found_folders.sort(key=lambda x: os.path.getmtime(os.path.join(parent_dir, x)), reverse=True)
                actual_output_dir = os.path.join(parent_dir, found_folders[0])
                self.output_queue.put(("info", f"Using most recent folder: {found_folders[0]}"))
            ...
            for script in soup.find_all('script'):
                ...
                if src and 'mathjax' in src.lower():
                    # Try multiple possible MathJax paths
                    mathjax_paths = [
                        r"C:\git\mineru-latex-converter\node_modules\mathjax\es5\tex-svg.js",
                        "/mnt/c/git/mineru-latex-converter/node_modules/mathjax/es5/tex-svg.js"
                    ]
                    
                    mathjax_content = None
                    for mathjax_path in mathjax_paths:
                        if os.path.exists(mathjax_path):
                            try:
                                self.output_queue.put(("info", f"Reading MathJax from: {mathjax_path}"))
                                with open(mathjax_path, 'r', encoding='utf-8') as mj_file:
                                    mathjax_content = mj_file.read()
                                break
                            except Exception as e:
                                self.output_queue.put(("warning", f"Failed to read {mathjax_path}: {str(e)}"))
                    ...
                    if mathjax_content:
                        ...
                        self.output_queue.put(("warning", "MathJax file not found, using CDN"))
                        script['src'] = "https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-svg.js"
            
            if not mathjax_embedded:
                self.output_queue.put(("warning", "No MathJax script tag found to replace"))
